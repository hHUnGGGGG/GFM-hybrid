...
class KGIndexer:
    ...
    def index_data(self, dataset_cfg: DictConfig) -> None:
        ...
        This method performs two main tasks:
            ...
            2. Identify the query entities and supporting entities in training and testing data
               if available in the raw data directory
...
```
        ...
        processed_data_dir = os.path.join(root, data_name, "processed", "stage1")

        if not os.path.exists(processed_data_dir):
            os.makedirs(processed_data_dir)

        # Create KG index
        kg_txt_path = os.path.join(processed_data_dir, "kg.txt")
        if not os.path.exists(kg_txt_path):
            logger.info("Stage1 KG construction")
            kg = self.kg_constructor.create_kg(root, data_name)
            # encoding="utf-8" để giữ nguyên ký tự tiếng Việt trong kg.txt
            with open(kg_txt_path, "w", encoding="utf-8") as f:
                for triple in kg:
                    f.write(self.DELIMITER.join(triple) + "\n")
            logger.info(f"Saved KG with {len(kg)} triples to {kg_txt_path}")

        # Create document2entities index
        doc2entities_path = os.path.join(processed_data_dir, "document2entities.json")
        if not os.path.exists(doc2entities_path):
            logger.info("Stage1 Get document2entities")
            doc2entities = self.kg_constructor.get_document2entities(root, data_name)
            # ensure_ascii=False để giữ nguyên tiếng Việt thay vì encode thành \uXXXX
            with open(doc2entities_path, "w", encoding="utf-8") as f:
                json.dump(doc2entities, f, indent=4, ensure_ascii=False)
            logger.info(f"Saved document2entities to {doc2entities_path}")

        # Prepare training data
        train_raw_path = os.path.join(raw_data_dir, "train.json")
        train_processed_path = os.path.join(processed_data_dir, "train.json")
        if os.path.exists(train_raw_path) and not os.path.exists(train_processed_path):
            logger.info(f"Preparing {train_raw_path}")
            ...
            logger.info(f"Saved train data ({len(train_data)} samples) to {train_processed_path}")

        # Prepare testing data
        test_raw_path = os.path.join(raw_data_dir, "test.json")
        test_processed_path = os.path.join(processed_data_dir, "test.json")
        if os.path.exists(test_raw_path) and not os.path.exists(test_processed_path):
            logger.info(f"Preparing {test_raw_path}")
            ...
            logger.info(f"Saved test data ({len(test_data)} samples) to {test_processed_path}")