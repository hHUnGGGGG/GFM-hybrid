...
from gfmrag_hybrid.kg_construction.openie_extraction_instructions import (
    ner_prompts,
    openie_post_ner_prompts,
)
from gfmrag_hybrid.kg_construction.utils import extract_json_dict
...
class LLMOPENIEModel(BaseOPENIEModel):
    ...
    def ner(self, text: str) -> list:
        ...
        logger.debug(f"NER Prompt: {ner_messages.to_messages()}")
        try:
            if isinstance(self.client, ChatOpenAI):  # JSON mode
                ...
                logger.debug(f"NER Raw Response: {response_content}")
    ...
    def openie_post_ner_extract(self, text: str, entities: list) -> str:
        ...
        logger.debug(f"OpenIE Prompt: {openie_messages.to_messages()}")
        try:
            if isinstance(self.client, ChatOpenAI):  # JSON mode
                chat_completion = self.client.invoke(
                ...
                response_content = chat_completion.content
                logger.debug(f"OpenIE Raw Response: {chat_completion.content}")
            elif isinstance(self.client, ChatOllama) or isinstance(
                self.client, ChatLlamaCpp
            ):
                response_content = self.client.invoke(
