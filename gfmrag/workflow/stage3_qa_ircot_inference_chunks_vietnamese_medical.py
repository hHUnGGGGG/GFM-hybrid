...
from gfmrag.gfmrag_retriever_with_entity_scores import GFMRetrieverWithEntityScores
...
from gfmrag.utils.qa_utils import (
...
class BM25Searcher:
    ...
    def search(self, queries: List[str], top_k: int = 50) -> List[Dict]:
        """RRF search với nhiều query string (batch numpy)."""
    ...
    def search_with_entity_scores(
        ...
        """BM25 search dùng tên entity làm query, trọng số norm_score (batch numpy)."""
    ...
    def search_standard(self, query: str, top_k: int = 50) -> List[Dict]:
        ...
        trả về điểm gốc của thuật toán.
        """
        ...
        # Lấy điểm gốc của thuật toán BM25
        ...
        for idx in top_indices:
            ...
            if score == 0:
                continue
            chunk = self.all_chunks[int(idx)].copy()
            # Lưu thẳng điểm gốc vào trường bm25_score
