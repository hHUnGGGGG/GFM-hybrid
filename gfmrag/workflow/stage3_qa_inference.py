...
def ans_prediction(
    ...
    with open(os.path.join(output_dir, "prediction.jsonl"), "w", encoding="utf-8") as f:
...
def main(cfg: DictConfig) -> None:
    ...
    # === THÊM MỚI TỪ ĐÂY ĐỂ GIỚI HẠN SỐ LƯỢNG MẪU TEST ===
    # Lấy thông số max_test_samples từ cfg hoặc cfg.test
    max_samples = cfg.get("max_test_samples") or (cfg.test.get("max_test_samples") if "test" in cfg else None)

    if max_samples is not None:
        logger.info(f"Limiting test set to maximum {max_samples} samples.")

        # 1. Truncate phần Pytorch Dataset (dùng cho model retrieval chạy batch)
        train_data, test_data = qa_data._data
        limited_test_data = torch_data.Subset(test_data, range(min(len(test_data), max_samples)))
        qa_data._data = (train_data, limited_test_data)

        # 2. Truncate phần Raw Test Data list (dùng để gán answer prediction sau đó)
        qa_data.raw_test_data = qa_data.raw_test_data[:max_samples]
    # ====================================================
    ...
    if cfg.test.retrieved_result_path:
        ...
        # === THÊM MỚI: Cắt list retrieved kết quả cache ổ cứng (nếu có) ===
        if max_samples is not None:
            retrieval_result = retrieval_result[:max_samples]
        # ================================================================
    else:
